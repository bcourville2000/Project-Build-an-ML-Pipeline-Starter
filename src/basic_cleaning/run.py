...
def go(args):
    ...
    idx = df['longitude'].between(-74.25, -73.50) & df['latitude'].between(40.5, 41.2) #made changes again to code to demonstrate addressing functionality issues within code
    df = df[idx].copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min_price", 
        type = float,
        help = "minimum house price considered",
        required = True
    )
    ...
    parser.add_argument(
        "--max_price",
        type = float,
        help = "maximum house price considered",
        required = True
    )
